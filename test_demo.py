...
def select_model(args, device):
    ...
    if model_id == 1:
        # Gachon Team
            import DRFDN as drfdn
            name, data_range = f"{model_id:02}_DRFDN_residual", 255.0
            model_path = os.path.join('model_zoo', 'model_deeprfdn.pt')# here we provide already pretrained model for scale factor x4
            model = drfdn.VDSR(args)
...
def main(args):
    ...
    utils_logger.logger_info("drfdn", log_path="drfdn.log")
    logger = logging.getLogger("drfdn")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("drfdn")
    parser.add_argument("--data_dir", default="dataset/Set5/Set5", type=str)
